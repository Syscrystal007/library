...
public class LendDao {
    ...
    public ArrayList<Lend> queryLend(final String type, final String searchWord){

        Map<String,String> map = new HashMap<>();
        map.put("ser_num","queryLendBySernum");
        map.put("book_id","queryLendByBookId");
        map.put("reader_id","queryLendReaderId");

        String search = null;
        if(type.equals("reader_id")){
            search = "%"+ searchWord + "%";
        }else{
            search = searchWord;
        }

        List<Lend> result = sqlSessionTemplate.selectList(NAMESPACE + map.get(type),search);
        return (ArrayList<Lend>) result;
